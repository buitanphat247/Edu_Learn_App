import { Ionicons } from "@expo/vector-icons";
import React from "react";
import { Image, Text, TouchableOpacity, View } from "react-native";

const AVATAR_URL =
  "https://cellphones.com.vn/sforum/wp-content/uploads/2024/02/anh-thien-nhien-4.jpg";

interface ProfileHeaderProps {
  onViewPress?: () => void;
  onEditPress?: () => void;
}

export default function ProfileHeader({
  onViewPress,
  onEditPress,
}: ProfileHeaderProps) {
  return (
    <View className="items-center pb-6">
      {/* Avatar with dashed border */}
      <View className="relative mb-4">
        <View className="w-24 h-24 rounded-full border-2 border-dashed border-purple-500 items-center justify-center overflow-hidden">
          <Image
            source={{ uri: AVATAR_URL }}
            className="w-full h-full rounded-full"
            resizeMode="cover"
          />
        </View>
        {/* Camera icon overlay */}
        <TouchableOpacity className="absolute bottom-0 right-0 w-8 h-8 bg-white rounded-full items-center justify-center border border-gray-200">
          <Ionicons name="camera" size={16} color="#9333EA" />
        </TouchableOpacity>
      </View>

      {/* Name */}
      <Text className="text-xl font-bold text-gray-900 mb-1">Nguyễn Văn A</Text>

      {/* Email */}
      <Text className="text-sm text-gray-500 mb-6">
        nguyen.van.a@example.com
      </Text>

      {/* Action Buttons */}
      <View className="flex-row gap-3">
        {/* View Profile Button */}
        <TouchableOpacity
          onPress={onViewPress}
          className="flex-row items-center bg-white border border-purple-500 px-5 py-2.5 rounded-xl"
          activeOpacity={0.8}
        >
          <Ionicons name="eye-outline" size={18} color="#9333EA" />
          <Text className="text-purple-600 font-semibold ml-2">Xem hồ sơ</Text>
        </TouchableOpacity>

        {/* Edit Profile Button */}
        <TouchableOpacity
          onPress={onEditPress}
          className="flex-row items-center bg-purple-600 px-5 py-2.5 rounded-xl"
          activeOpacity={0.8}
        >
          <Ionicons name="pencil" size={18} color="#FFFFFF" />
          <Text className="text-white font-semibold ml-2">Chỉnh sửa</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
}
